...
// IM_VEC2_CLASS_EXTRA to create implicit cast from/to our preferred type. Add
// '#define IMGUI_DEFINE_MATH_OPERATORS' in your imconfig.h file to benefit from
// courtesy maths operators for those types.
...
IMGUI_API void Image(ImTextureID user_texture_id, const ImVec2 &image_size,
...
//  Call EndPopup() afterwards if returned true. ImGuiWindowFlags are forwarded
//  to the window.
...
// (Legacy: bit 0 must always correspond to ImGuiChildFlags_Border to be
...
//   - You may also use ImGuiChildFlags_AlwaysAutoResize to force an update even
...
enum ImGuiChildFlags_ {
  ...
  ImGuiChildFlags_ResizeX =
      1 << 2, // Allow resize from right border (layout direction). Enable .ini
              // saving (unless ImGuiWindowFlags_NoSavedSettings passed to
              // window flags)
...
enum ImGuiTreeNodeFlags_ {
  ...
  // ImGuiTreeNodeFlags_NoScrollOnOpen     = 1 << 15,  // FIXME: TODO: Disable
...
// 1' argument instead of 'ImGuiPopupFlags flags',
//   we need to treat small flags values as a mouse button index, so we encode
//   the mouse button in the first few bits of the flags. It is therefore
//   guaranteed to be legal to pass a mouse button index in ImGuiPopupFlags.
...
enum ImGuiPopupFlags_ {
  ...
  ImGuiPopupFlags_NoReopen =
      1
      << 5, // For OpenPopup*(), BeginPopupContext*(): don't reopen same popup
            // if already open (won't reposition, won't reinitialize navigation)
  // ImGuiPopupFlags_NoReopenAlwaysNavInit = 1 << 6,   // For OpenPopup*(),
  // BeginPopupContext*(): focus and initialize navigation even when not
  // reopening.
  ImGuiPopupFlags_NoOpenOverExistingPopup =
      1 << 7, // For OpenPopup*(), BeginPopupContext*(): don't open if there's
              // already a popup at the same level of the popup stack
  ImGuiPopupFlags_NoOpenOverItems =
      1 << 8, // For BeginPopupContextWindow(): don't return true when hovering
              // items, only when hovering empty space
  ImGuiPopupFlags_AnyPopupId = 1 << 10, // For IsPopupOpen(): ignore the ImGuiID
                                        // parameter and test for any popup.
  ImGuiPopupFlags_AnyPopupLevel =
      1 << 11, // For IsPopupOpen(): search/test at any level of the popup stack
...
enum ImGuiTabItemFlags_ {
  ...
  ImGuiTabItemFlags_UnsavedDocument =
      1 << 0, // Display a dot next to the title + set
...
// Since 1.90, defining IMGUI_DISABLE_OBSOLETE_FUNCTIONS automatically defines
// IMGUI_DISABLE_OBSOLETE_KEYIO as well.
#if defined(IMGUI_DISABLE_OBSOLETE_FUNCTIONS) &&                               \
    !defined(IMGUI_DISABLE_OBSOLETE_KEYIO)
...
// are submitted via io.AddInputCharacter(). The keyboard key enum values are
// named after the keys on a standard US keyboard, and on other keyboard types
// the keys reported may not match the keycaps.
enum ImGuiKey : int {
  // Keyboard
  ImGuiKey_None = 0,
  ...
  ImGuiKey_ModSuper =
      ImGuiMod_Super, // Renamed in 1.89
...
// Enumeration for ImGui::SetNextWindow***(), SetWindow***(), SetNextItem***()
...
struct ImGuiIO {
  ...
  // - The Item Picker tool will be available regardless of this being enabled,
  // in order to maximize its discoverability.
  // - Requires a debugger being attached, otherwise IM_DEBUG_BREAK() options
  // will appear to crash your application.
  //   e.g. io.ConfigDebugIsDebuggerPresent = ::IsDebuggerPresent() on Win32, or
  //   refer to ImOsIsDebuggerPresent() imgui_test_engine/imgui_te_utils.cpp for
  //   a Unix compatible version).
  ...
  // called in tandem, regardless of return value of BeginXXX()
  // - This is inconsistent with other BeginXXX functions and create confusion
  // for many users.
  // - We expect to update the API eventually. In the meanwhile we provide tools
  // to facilitate checking user-code behavior.
  ...
  // - May facilitate interactions with a debugger when focus loss leads to
  ...
  // void*     ImeWindowHandle;                    // [Obsoleted in 1.87] Set
  // ImGuiViewport::PlatformHandleRaw instead. Set this to your HWND to get
...
static inline bool BeginChildFrame(ImGuiID id, const ImVec2 &size,
                                   ImGuiWindowFlags window_flags = 0) {
...
// bool border, ImGuiWindowFlags window_flags){ return BeginChild(str_id,
// size_arg, border ? ImGuiChildFlags_Border : ImGuiChildFlags_None,
// window_flags); } // Unnecessary as true == ImGuiChildFlags_Border static
// inline bool BeginChild(ImGuiID id, const ImVec2& size_arg, bool border,
// ImGuiWindowFlags window_flags)        { return BeginChild(id, size_arg,
// border ? ImGuiChildFlags_Border : ImGuiChildFlags_None, window_flags);     }
// // Unnecessary as true == ImGuiChildFlags_Border
...
// IMGUI_API void      CalcListClipping(int items_count, float items_height,
